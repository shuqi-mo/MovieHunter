...
$(function (){
    // var flag = true;
    // $("blink search-field").on('focus', function () {
    //     $(this).prop('val', '');
    // })
    //
    // $("blink search-field").on('blur', function () {
    //     $(this).prop('val', "Enter search here");
    // })
    $(".blink").on('keydown', function (e) {
        var movieId = $(this).val();
        var baseUrl = './movie.html?movieId=' + movieId;
        if (e.keyCode == 13) {
            window.location.assign('./movie.html?movieId=' + $(this).val());
            addMovieDetails("movie-details-container", movieId, baseUrl);
            addRelatedMovies("#recPage", "relatedMovies", movieId, baseUrl);
        }
    })
})
...
function addGenreRow(pageId, rowName, rowId, size, baseUrl) {
    ...
    $.getJSON(baseUrl + "getrecommendation?genre="+rowName+"&size="+size+"&sortby=releaseYear", function(result){
